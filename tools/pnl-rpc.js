...
import { getDLMM, getConnection } from "./dlmm-adapter.js";
import { getTrackedPosition, markOutOfRange, markInRange, minutesOutOfRange } from "../state.js";

// ─── Public-infra PnL source (opt-in via config.pnl.source = "rpc") ────
// Live position value (current liquidity + claimable fees) is read ON-CHAIN
// via the Meteora DLMM SDK on the configured RPC_URL. Deposit history (cost
// basis, withdrawals, claimed fees) comes from the Meteora /pnl API — its
// precomputed live pnl/balances are intentionally ignored, only the deposit
// ledger is used. Token USD prices come from Jupiter. No LPAgent / Agent
// Meridian relay dependency.
//
// Output shape matches getMyPositions()'s position objects field-for-field
// (see tools/dlmm.js) so callers don't need to know which source produced it.
...
// Deliberately not shared with tools/dlmm.js's own copy of this fetch to
// avoid a circular import (dlmm.js -> pnl-rpc.js -> dlmm.js); it's a small,
// self-contained HTTP call.
...
// signature changes or config.pnl.depositCacheTtlSec lapses.
...
export function buildPosition(f, prices, solUsd, meteora, solMode) {
  ...
  // pnl_pct_diff is a logged diagnostic only, never used to gate exits here:
  // Meteora's precomputed pct comes from the (possibly stale) deposit cache,
  // while ourPct is fresh every call — on a fast move the gap inflates and
  // would falsely suppress STOP_LOSS/TRAILING_TP exactly when they matter.
  ...
  // On-chain amounts are authoritative; a tick is "suspicious" (don't act on
  // it) only when we couldn't price it — guards against a Jupiter outage
  // (missing solUsd/priceX collapsing balances into a false STOP_LOSS) or a
  // missing Meteora deposit record (zero cost basis producing garbage pnl).
  ...
  if (pnlPctSuspicious) {
    log("positions_warn", `${f.position.slice(0, 8)} suspicious RPC PnL tick — priceMissing=${priceMissing} depositsMissing=${depositsMissing} (solUsd=${solUsd}, priceX=${priceX})`);
  }
  ...
  else markOutOfRange(f.position, { active_bin: f.active, lower_bin: f.lower, upper_bin: f.upper });
...
// ─── Main entry: compute positions straight from chain + public APIs ───
// Returns the same shape as getMyPositions(), or throws so the caller
// (tools/dlmm.js) can fall back to the relay/LPAgent path.
